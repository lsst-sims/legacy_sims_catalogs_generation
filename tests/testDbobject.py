...
def createNonsenseDB():
    ...
    """
    if os.path.exists('testDbobjectNonsenseDB.db'):
        os.unlink('testDbobjectNonsenseDB.db')
    
    conn = sqlite3.connect('testDbobjectNonsenseDB.db')
    c = conn.cursor()
    try:
...
class myNonsenseDB(DBObject):
    ...
    tableid = 'test'
    idColKey = 'NonsenseId' 
    dbAddress = 'sqlite:///testDbobjectNonsenseDB.db'
    raColName = 'ra'
    decColName = 'dec'
...
class testDbobjectTestStars(myTestStars):
    objid = 'testDbobjectTeststars'
    dbAddress = 'sqlite:///testDbobjectDatabase.db'

class testDbobjectTestGalaxies(myTestGals):
    objid = 'testDbobjectTestgals'
    dbAddress = 'sqlite:///testDbobjectDatabase.db'

class DBObjectTestCase(unittest.TestCase):
  
    #Delete the test database if it exists and start fresh.
    if os.path.exists('testDbobjectDatabase.db'):
        print "deleting database"
        os.unlink('testDbobjectDatabase.db')
    tu.makeStarTestDB(filename='testDbobjectDatabase.db',size=100000, seedVal=1)
    tu.makeGalTestDB(filename='testDbobjectDatabase.db',size=100000, seedVal=1)
    createNonsenseDB()
    ...
    def testDbObj(self):
        mystars = DBObject.from_objid('testDbobjectTeststars')
        mygals = DBObject.from_objid('testDbobjectTestgals')
    ...
    def testClassVariables(self):
        ...
        mystars = DBObject.from_objid('testDbobjectTeststars')
        myNonsense = DBObject.from_objid('Nonsense')
        mygalaxies = DBObject.from_objid('testDbobjectTestgals')
        
        self.assertEqual(mystars.raColName,'ra')
        self.assertEqual(mystars.decColName,'decl')
        self.assertEqual(mystars.idColKey,'id')
        self.assertEqual(mystars.dbAddress,'sqlite:///testDbobjectDatabase.db')
        ...
        self.assertEqual(mystars.objid,'testDbobjectTeststars')
        ...
        self.assertEqual(mygalaxies.dbAddress,'sqlite:///testDbobjectDatabase.db')
        ...
        self.assertEqual(mygalaxies.objid,'testDbobjectTestgals')
        ...
        self.assertEqual(myNonsense.dbAddress,'sqlite:///testDbobjectNonsenseDB.db')
        ...
        self.assertTrue('testDbobjectTeststars' in DBObject.registry)
        self.assertTrue('testDbobjectTestgals' in DBObject.registry)
...
class fileDBObjectTestCase(unittest.TestCase):
    """
    This class will re-implement the tests from DBObjectTestCase,
    except that it will use a Nonsense DBObject loaded with fileDBObject
